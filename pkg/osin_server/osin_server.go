...
import (
	"avanpost_auth/pkg/osin_server/util"
	"fmt"
...
func InitOsinServer() {
	...
	sstorage := util.NewTestStorage()
	sstorage.SetClient("1234", &osin.DefaultClient{
		Id:          "1234",
		Secret:      "aabbccdd",
		RedirectUri: "http://localhost:3011/appauth",
		//RedirectUri: "http://localhost:14000/appauth",
	})
	server := osin.NewServer(sconfig, sstorage)
	...
	http.HandleFunc("/authorize", func(w http.ResponseWriter, r *http.Request) {
		...
		if ar := server.HandleAuthorizeRequest(resp, r); ar != nil {
			if !util.HandleLoginPage(ar, w, r) {
				return
			}
	...
	http.HandleFunc("/token", func(w http.ResponseWriter, r *http.Request) {
		...
		if ar := server.HandleAccessRequest(resp, r); ar != nil {
			...
			case osin.CLIENT_CREDENTIALS:
				ar.Authorized = true
			case osin.ASSERTION:
				if ar.AssertionType == "urn:osin.util.complete" && ar.Assertion == "osin.data" {
					ar.Authorized = true
				}
			}
	...
	http.HandleFunc("/appauth/code", func(w http.ResponseWriter, r *http.Request) {
		...
		if r.FormValue("doparse") == "1" {
			err := util.DownloadAccessToken(fmt.Sprintf("http://localhost:14000%s", aurl),
	...
	http.HandleFunc("/appauth/assertion", func(w http.ResponseWriter, r *http.Request) {
		...
		// build access code url
		aurl := fmt.Sprintf("/token?grant_type=assertion&assertion_type=urn:osin.util.complete&assertion=osin.data")
